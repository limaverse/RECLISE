...
switch ($action) {
        ...
        if (!empty($_FILES['attachments']['name'][0])) {
            ...
            foreach ($_FILES['attachments']['name'] as $i => $fname) {
                if ($_FILES['attachments']['error'][$i] === UPLOAD_ERR_OK) {
                    ...
                    $safeName = uniqid() . '_' . basename($fname);
                    move_uploaded_file($tmp, $uploadDir . $safeName);
                    $stmt2 = $pdo->prepare("INSERT INTO request_attachments (request_id, file_name, file_path) VALUES (?, ?, ?)");
                    $stmt2->execute([$reqId, $fname, 'uploads/' . $safeName]);
                }
            }
        }
    ...
    case 'registerTraining':
        if ($role !== 'user') json_error('Only users can register for training');
        $sessionId = (int)($_POST['sessionId'] ?? 0);
        if (!$sessionId) json_error('Session ID required');

        $check = $pdo->prepare("SELECT id FROM training_registrations WHERE user_id = ? AND session_id = ?");
        $check->execute([$userId, $sessionId]);
        if ($check->fetch()) {
            $pdo->prepare("DELETE FROM training_registrations WHERE user_id = ? AND session_id = ?")->execute([$userId, $sessionId]);
            json_response(['status' => 'success', 'action' => 'unregistered']);
        } else {
            $pdo->prepare("INSERT INTO training_registrations (user_id, session_id) VALUES (?, ?)")->execute([$userId, $sessionId]);
            json_response(['status' => 'success', 'action' => 'registered']);
        }
    ...
    case 'addTrainingSession':
        ...
        $stmt->execute([$title, $desc, $date, $userId]);
    ...
    case 'deleteBox':
        if ($role !== 'admin') json_error('Unauthorized');
        $id = (int)($_POST['id'] ?? 0);
        if (!$id) json_error('Box ID required');
        $pdo->prepare("DELETE FROM distribution_box_members WHERE box_id = ?")->execute([$id]);
    ...
    case 'updateGuide':
        ...
        $title = trim($_POST['title'] ?? '');
        $content = trim($_POST['content'] ?? '');
        if (!$id || !$title) json_error('Required fields missing');

        $pdo->prepare("UPDATE guides_coordinators SET full_name = ?, email = ? WHERE id = ?")->execute([$title, $content, $id]);
        json_response(['status' => 'success']);
    ...
    case 'addCorrespondence':
        ...
        $stmt = $pdo->prepare("INSERT INTO correspondences (name, type) VALUES (?, ?)");
        $stmt->execute([$name, $type]);
        log_audit($pdo, 'ADD_CORRESPONDENCE', "Correspondence added: $name", $userId);
        json_response(['status' => 'success']);
    ...
    case 'addSubTask':
        ...
        $pdo->prepare("INSERT INTO referentials (type, value) VALUES ('subTasks', ?)")->execute([$label]);
        json_response(['status' => 'success']);

    case 'deleteSubTask':
        if ($role !== 'admin') json_error('Unauthorized');
        $label = trim($_POST['label'] ?? '');
        $pdo->prepare("DELETE FROM referentials WHERE type = 'subTasks' AND value = ?")->execute([$label]);
        json_response(['status' => 'success']);

    case 'addClosureMotive':
        if ($role !== 'admin') json_error('Unauthorized');
        $label = trim($_POST['label'] ?? '');
        if (!$label) json_error('Label required');

        $pdo->prepare("INSERT INTO referentials (type, value) VALUES ('closureMotives', ?)")->execute([$label]);
        json_response(['status' => 'success']);

    case 'deleteClosureMotive':
        if ($role !== 'admin') json_error('Unauthorized');
        $label = trim($_POST['label'] ?? '');
        $pdo->prepare("DELETE FROM referentials WHERE type = 'closureMotives' AND value = ?")->execute([$label]);
        json_response(['status' => 'success']);
    ...
    case 'addBox':
        ...
        $pdo->prepare("INSERT INTO distribution_boxes (name, location) VALUES (?, ?)")->execute([$name, $location]);
        $boxId = $pdo->lastInsertId();
        if ($members) {
            $memberList = array_map('trim', explode(',', $members));
            foreach ($memberList as $m) {
                if ($m) $pdo->prepare("INSERT INTO distribution_box_members (box_id, member_name) VALUES (?, ?)")->execute([$boxId, $m]);
            }
        }
        log_audit($pdo, 'ADD_BOX', "Box added: $name", $userId);
        json_response(['status' => 'success']);
    ...
    case 'addTechIssue':
        ...
        $pdo->prepare("INSERT INTO technical_issues (title, description, priority, created_by) VALUES (?, ?, ?, ?)")->execute([$title, $desc, $priority, $userId]);
        log_audit($pdo, 'CREATE_TECHNICAL_ISSUE', "Issue added: " . substr($desc, 0, 50), $userId);
        json_response(['status' => 'success']);

    case 'resolveTechIssue':
        if ($role !== 'admin') json_error('Unauthorized');
        $id = (int)($_POST['id'] ?? 0);
        $pdo->prepare("UPDATE technical_issues SET status = 'resolved', resolved_at = NOW() WHERE id = ?")->execute([$id]);
        json_response(['status' => 'success']);

    case 'deleteTechIssue':
        if ($role !== 'admin') json_error('Unauthorized');
        $id = (int)($_POST['id'] ?? 0);
    ...
    case 'updateSystemSetting':
        ...
        $key = $_POST['key'] ?? '';
        $value = $_POST['value'] ?? '';
        if (!$key) json_error('Key required');
    ...
    case 'getData':
        if ($role === 'user') {
            $stmt = $pdo->prepare("SELECT id, email, full_name, role, phone, department, status, joined_at, avatar FROM users WHERE id = ?");
            $stmt->execute([$userId]);
            ...
            $stmt = $pdo->query("SELECT id, email, full_name, role, phone, department, status, joined_at, avatar FROM users");
            $users = to_camel_all($stmt->fetchAll());
    ...
    case 'registerTraining':
        if ($role !== 'user') json_error('Unauthorized');
        $id = (int)($_POST['id'] ?? 0);
        $stmt = $pdo->prepare("INSERT INTO training_registrations (session_id, user_id, status) VALUES (?, ?, 'registered') ON DUPLICATE KEY UPDATE status='registered'");
    ...
    case 'addGuide':
        if ($role !== 'support' && $role !== 'admin') json_error('Unauthorized');
        $name = $_POST['name'] ?? ''; $email = $_POST['email'] ?? ''; $type = $_POST['type'] ?? 'guide';
        $stmt = $pdo->prepare("INSERT INTO guides_coordinators (full_name, email, type) VALUES (?, ?, ?)");
    ...
    case 'editGuide':
        if ($role !== 'support' && $role !== 'admin') json_error('Unauthorized');
        $id = (int)($_POST['id'] ?? 0); $name = $_POST['name'] ?? ''; $email = $_POST['email'] ?? '';
    ...
    case 'getAssistDetail':
        $id = (int)($_GET['id'] ?? 0);
        $stmt = $pdo->prepare("SELECT a.*, u.full_name as userName FROM assist a JOIN users u ON a.user_id = u.id WHERE a.id = ?");
        $stmt->execute([$id]);
        json_response(to_camel($stmt->fetch()));

    case 'addCorrespondence':
        if ($role !== 'support' && $role !== 'admin') json_error('Unauthorized');
        $title = $_POST['title'] ?? ''; $type = $_POST['type'] ?? ''; $content = $_POST['content'] ?? '';
        $stmt = $pdo->prepare("INSERT INTO correspondences (title, type, content, created_by) VALUES (?, ?, ?, ?)");
        $stmt->execute([$title, $type, $content, $userId]);
        json_response(['status' => 'success']);
    ...
    case 'addTechnicalIssue':
        if ($role !== 'support' && $role !== 'admin') json_error('Unauthorized');
        $title = $_POST['title'] ?? ''; $desc = $_POST['description'] ?? ''; $priority = $_POST['priority'] ?? 'medium';
    ...
    case 'addReferential':
        if ($role !== 'admin') json_error('Unauthorized');
        $type = $_POST['type'] ?? ''; $value = $_POST['value'] ?? '';
    ...
    case 'resetPassword':
        if ($role !== 'admin') json_error('Unauthorized');
        $id = (int)($_POST['id'] ?? 0); $pw = $_POST['password'] ?? '';
        if (strlen($pw) < 6) json_error('Password too short');
        $pdo->prepare("UPDATE users SET password_hash = ? WHERE id = ?")->execute([hash('sha256', $pw), $id]);
        json_response(['status' => 'success']);
