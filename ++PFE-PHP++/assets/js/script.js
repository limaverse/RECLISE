const RecLise = (function () {
  ...
  // Initialize
  function init() {
    const user = window.currentUser || { id: 1, role: 'user' };
    const lang = window.appLang || 'fr';
    const theme = window.appTheme || 'dark';
  ...
  // View Assist Details
  function viewAssistDetails(id) {
    fetch('../ajax/api.php?action=getAssistDetail&id=' + id)
    .then(r => r.json())
    .then(d => {
      openModal('<h3>Assist Details</h3><p><strong>User:</strong> ' + d.userName + '</p><p><strong>Issue:</strong> ' + d.issue + '</p><p><strong>Status:</strong> ' + d.status + '</p><p><strong>Resolution:</strong> ' + (d.resolution || 'N/A') + '</p><button class="btn btn-outline-neon w-100 mt-3" onclick="RecLise.closeModal()">Close</button>');
    });
  }

  // Show Add Correspondence Modal
  function showAddCorrespondenceModal() {
    openModal('<h3>Add Correspondence</h3><form onsubmit="event.preventDefault(); RecLise.addCorrespondence();"><div class="mb-3"><label class="form-label">Title</label><input type="text" class="form-control" id="corrTitle" required></div><div class="mb-3"><label class="form-label">Type</label><input type="text" class="form-control" id="corrType"></div><div class="mb-3"><label class="form-label">Content</label><textarea class="form-control" id="corrContent" rows="4"></textarea></div><button type="submit" class="btn btn-neon w-100">Add</button></form>');
  }
  ...
  function showResetPasswordModal(id) {
    openModal('<h3>Reset Password</h3><form onsubmit="event.preventDefault(); RecLise.resetPassword(' + id + ');"><div class="mb-3"><label class="form-label">New Password</label><input type="password" class="form-control" id="newPassword" required minlength="6"></div><button type="submit" class="btn btn-neon w-100">Reset Password</button></form>');
  }
